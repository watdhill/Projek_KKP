  ...
  try {
    ...
    if (type === "format_laporan") {
      const [details] = await pool.query(
        "SELECT field_id, id AS order_index FROM format_laporan_detail WHERE format_laporan_id = ? ORDER BY id ASC",
        [req.params.id],
      );
...
exports.updateMasterData = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        "Error mengupdate master data: " + (error.sqlMessage || error.message),
      error: error.message,
