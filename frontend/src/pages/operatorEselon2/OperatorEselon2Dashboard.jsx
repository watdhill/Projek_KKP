...
function OperatorEselon2Dashboard() {
  ...
  // Get eselon2_id from localStorage (set during login)
  const userInfo = JSON.parse(localStorage.getItem('userInfo') || '{}');
  const eselon2_id = userInfo.eselon2_id;
    ...
    <section className="page-section" style={{ padding: '24px', backgroundColor: '#f8fafc', minHeight: '100vh' }}>
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ margin: 0, color: '#64748b', fontSize: '14px' }}>
          Ringkasan statistik dan monitoring aplikasi unit {userInfo.nama_eselon2 || 'Anda'}
        </p>
      </div>
                      ...
                      <div style={{ flex: 1, position: 'relative', height: '32px', backgroundColor: '#f1f5f9', borderRadius: '6px', overflow: 'hidden' }}>
                        <div style={{
                          ...
                          backgroundColor: item.nama.toLowerCase().includes('aktif') && !item.nama.toLowerCase().includes('tidak')
                            ? 'rgb(16, 185, 129)'
                            : item.nama.toLowerCase().includes('tidak aktif')
                              ? 'rgb(239, 68, 68)'
                              : 'rgb(156, 163, 175)',
